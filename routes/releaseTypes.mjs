...
const createReleaseTypeValidation = [
    body("releaseTypeName")
        .notEmpty()
        .matches(/^[A-Za-z0-9'"]/),
];

// validation for specificying single releaseType by id
const releaseTypeByIdValidation = [param("releaseTypeID").notEmpty().escape()];
...
const deleteReleaseType = (req, res) => {
    ...
    const data = matchedData(req);
    const releaseTypeID = data.releaseTypeID;
    const query = `DELETE FROM Release_Types
        WHERE release_type_id = ${releaseTypeID};`;
...
export default {
    getReleaseTypes,
    getReleaseTypesValidation,
    createReleaseType,
    createReleaseTypeValidation,
    updateReleaseType,
    updateReleaseTypeValidation,
    deleteReleaseType,
    releaseTypeByIdValidation,
};
